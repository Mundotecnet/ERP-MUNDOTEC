...
export const SO_STATUSES = ['DRAFT', 'CONFIRMED', 'INVOICED', 'CANCELLED'] as const;
export type SoStatus = (typeof SO_STATUSES)[number];

export interface CreateSoLineBody {
  productId?: unknown;
  quantity?: unknown;
  unitPrice?: unknown;
...
export interface ParsedCreateSalesOrder {
  customerId: bigint;
...
export type UpdateSalesOrderBody = CreateSalesOrderBody;

export interface ParsedUpdateSalesOrder {
  customerId?: bigint;
  branchId?: bigint | null;
  salespersonId?: bigint | null;
  orderNumber?: string;
...
function parseLineBody(raw: unknown, idx: number): ParsedCreateSoLine {
  if (raw === null || typeof raw !== 'object') {
    throw new BadRequestException(`Línea #${idx + 1} debe ser un objeto.`);
  }
  const r = raw as CreateSoLineBody;
  return {
...
function parseLinesArray(value: unknown, requireNonEmpty: boolean): ParsedCreateSoLine[] {
...
export function parseCreateSalesOrderBody(body: CreateSalesOrderBody): ParsedCreateSalesOrder {
  return {
...
export function parseUpdateSalesOrderBody(body: UpdateSalesOrderBody): ParsedUpdateSalesOrder {
  const out: ParsedUpdateSalesOrder = {};
  if (body.customerId !== undefined) out.customerId = requireBigInt(body.customerId, 'customerId');
...
export interface ParsedListSalesOrdersQuery {
...
export function parseListSalesOrdersQuery(q: ListSalesOrdersQuery): ParsedListSalesOrdersQuery {
  let status: SoStatus | null = null;
  if (q.status !== undefined && q.status !== '') {
    if (!SO_STATUSES.includes(q.status as SoStatus)) {
      throw new BadRequestException(
        `Parámetro "status" debe ser uno de: ${SO_STATUSES.join(', ')}.`,
      );
    }
    status = q.status as SoStatus;
  }
  return {
    status,
