    ...
    mutationFn: async () => {
      const payload = {
        ...
        lines: so.lines.map((l) => ({
          ...
          unitPrice: l.unitPrice,
          taxRate: l.taxRate,
          // PR-38 — propaga el nivel de la línea de OV a la línea de factura.
          priceListId: l.priceListId,
        })),
      };
...
// ============================================================================
// PR-38 — Fila de línea de OV con selector de nivel + autocompletar + margen
// ============================================================================
// Espejo del LineRow de quotations.tsx (PR-37). Producto es REQUERIDO en OV
// (no hay líneas libres), el resto del comportamiento es idéntico.
function LineRow(props: {
  idx: number;
  register: UseFormRegister<SoFormValues>;
  watch: UseFormWatch<SoFormValues>;
  setValue: UseFormSetValue<SoFormValues>;
  ...
  return (
    <tr className="border-b last:border-b-0" data-testid={`so-line-row-${idx}`}>
      ...
      <td
        ...
        data-testid={`so-line-margin-${idx}`}
      >
